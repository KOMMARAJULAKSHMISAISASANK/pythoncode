...
def main():
  ...
  if selected_box == 'emotion recognition':
    st.sidebar.success('to try yourself by adding a audio file:')
    application()
  if selected_box == 'view source code':
    st.code(get_file_content_as_string("code.py"))
    
@st.cache(show_spinner=False)
def get_file_content_as_string(path):
  url = 'https://github.com/KOMMARAJULAKSHMISAISASANK/pythoncode/blob/main/'+path
...
def predict(model_filepath):
  emotions={1:'neutral', 2: 'calm', 3:'happy', 4: 'sad', 5: 'angry', 6: 'fearfull', 7:'disgust', 8:'surprised'}
